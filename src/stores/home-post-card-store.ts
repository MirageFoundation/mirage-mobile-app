...
export const useHomePostCardStore = create<HomePostCardState>((set) => ({
  ...
  setActiveVideoPostId: (feedScreen, postId) =>
    set((state) => {
      ...
      return {
        activeVideoPostIds: { ...state.activeVideoPostIds, [feedScreen]: postId },
      };
    }),
  setVisibleVideoPostIds: (feedScreen, postIds) =>
    set((state) => {
      ...
      return {
        visibleVideoPostIds: { ...state.visibleVideoPostIds, [feedScreen]: postIds },
      };
    }),
  setVideoViewability: (feedScreen, visibleIds, warmIds, activeId) =>
    set((state) => {
      ...
      if (visibleChanged) {
        updates.visibleVideoPostIds = {
          ...state.visibleVideoPostIds,
          [feedScreen]: visibleIds,
        };
      }
      if (warmChanged) {
        updates.warmVideoPostIds = {
          ...state.warmVideoPostIds,
          [feedScreen]: warmIds,
        };
      }
      if (activeChanged) {
        updates.activeVideoPostIds = {
...
export const useIsPostVisible = (postId: string, feedScreen: string) =>
  useHomePostCardStore((state) =>
    state.visibleVideoPostIds[feedScreen]?.has(postId) ?? false,
  );
...
export const useFeedActive = (screen: "home" | "following" | "topic") =>
  useHomePostCardStore(
    (state) => state.activeFeedScreen === screen && !state.sideMenuOpen,
  );
